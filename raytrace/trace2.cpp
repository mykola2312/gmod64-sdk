...
static uint32 MapDistanceToPixel(float t)
{
	...
	int a=(int)(t*1000.0f); a&=0xff;
	int b=(int)(t*10.0f); b &=0xff;
	int c=(int)(t*.01f); c &=0xff;
...
void RayTracingEnvironment::ComputeVirtualLightSources(void)
{
	...
	for(int b=0;b<3;b++)
	{
		...
		for(int l=where_to_start;l<nl;l++)
		{
			...
			int n_desired=(int)(1.0f*LightList[l].m_Color.Length());
			if (LightList[l].m_Type==MATERIAL_LIGHT_SPOT)
				n_desired*=(int)(LightList[l].m_Phi/2.0f);
				...
				if (li.IsDirectionWithinLightCone(trial_dir))
				{
					...
					if ((rslt.HitIds[0]!=-1))
					{
						...
						float area_of_virtual_light=
							4.0f*(float)M_PI*SQ(SubFloat(rslt.HitDistance, 0))*(1.0f / n_desired);
						...
						{
							l1.m_Color*=area_of_virtual_light/(float)M_PI;
							l1.m_Range=0.0;
							l1.m_Falloff=1.0;
