                ...
                int OldSize, int DiffListSize, int &ResultListSize,uint8 *Output,uint32 OutSize)
{
  ...
  ResultListSize = (int)((ptrdiff_t)Output - (ptrdiff_t)obuf);
  ...
  while(walk<NewBlock+NewSize)
  {
    ...
    if (walk<NewBlock+NewSize-MIN_MATCH_LEN)
    {
      ...
      for(BlockPtr *b=HashedMatches[hash1];b;b=b->Next)
      {
        ...
        {
          size_t max_mlength=vmin(65535,(size_t)(OldBlock+OldSize-b->dataptr));
          max_mlength=vmin(max_mlength,(size_t)(NewBlock+NewSize-walk));
          size_t i;
...
//        printf("copy from %x to %x len=%d\n", match_of,outbuf-Output,longest);
  ...
  if (Blocks)
    delete[] Blocks;
  DiffListSize=(int)(outbuf-Output);
  return ret;
}
                        ...
                        int NewSize, int OldSize, int &DiffListSize,uint8 *Output,uint32 OutSize)
{
  ...
  while(walk<NewBlock+NewSize)
  {
    ...
    if (walk<NewBlock+NewSize-MIN_MATCH_LEN)
    {
      ...
      {
        int max_bytes_to_compare=vmin(NewBlock+NewSize-walk,OldBlock+OldSize-old_data_hash[hash1]);
        int nmatches;
